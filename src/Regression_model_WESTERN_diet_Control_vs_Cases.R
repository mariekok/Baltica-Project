...
# load the data
load("~/Documents/PhD_2016-2019/projects/Baltica_Folder/Baltica/Baltica_project/results/data/BALTICA_MUVRdata_16042019.rda")
...
dim(Controls_W)
dim(Cases_W)
Control_label <-t(rep(0,dim(Controls_W)[1]))
Case_label <- t(rep(1,dim(Cases_W)[1]))
New_Group <-c(Control_label,Case_label)

# combine Case and control data with the new labels

new_data2 <- rbind.data.frame(Controls_W,Cases_W)
...
Controls <-  subset(new.covariates.data, GROUP=="1" )


# combine all the cases together using the  subset function 

Cases <-  subset(new.covariates.data, GROUP=="2"  )
...
#write.csv(Control_Case_model_all_pvalues, file = paste0(output.path, "model/","Baltica_logisticregression_pvalues_WesternDIET_controlvscase_16042019.csv"), row.names = FALSE, na = "")
#write.csv(odds.CI, file = paste0(output.path, "model/","Baltica_logisticregression_OR_WesternDIET_controlvscase_16042019.csv"), row.names = FALSE, na = "")
all_results <-c(Control_Case_model_all_pvalues,odds.CI)

write.csv(all_results, file = paste0(output.path, "model/","ALL_Results_Baltica_logisticregression_WesternDIET_controlvscase_26062019.csv"), row.names = FALSE, na = "")
